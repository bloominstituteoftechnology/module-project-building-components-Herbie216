function moduleProject3() {
  ...
  function buildLearnerCard(learner, languages) {
    ...
    [nameP, idP, dobP, favLanguageP].forEach(p => {
      card.appendChild(p);
    });
  ...
  function buildFooter(footerData) {
    ...
    for (let social in footerData.socialMedia){
      let socialMediaLnk = document.createElement("a");
      socialMediaLnk.href = footerData.socialMedia[social];
      socialMediaLnk.textContent = social.charAt(0).toUpperCase() + social.slice(1);
      socialMediaDiv.appendChild(socialMediaLnk);
    }
